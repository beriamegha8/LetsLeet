class Solution {
    ...
    //modifying graph edges based on the provided conditions
    public int[][] modifiedGraphEdges(int n, int[][] edges, int source, int destination, int target) {
        //run dijkstra to find the current shortest path distance
        long dist = dijkstra(edges, n, source, destination);
      
        //if the current distance is less than the target
        if (dist < target) {
        ...
        //iterating through each edge to adjust weights
        for (int[] edge : edges) {
    ...
    private long dijkstra(int[][] edges, int n, int src, int dest) {
        ...
        long[] distances = new long[n]; // Distances array to store minimum distance to each vertex from source
        Arrays.fill(distances, INFINITY); // Initialize distances with infinity
        distances[src] = 0; // Distance to source from source is 0
